...
namespace WordRiddleFresh
{
    public partial class GameWindow : Window
    {
        ...
        public GameWindow(DBController database)
        {
            InitializeComponent();
            ...
            Console.WriteLine($"btnHint null? {btnHint == null}");
            Console.WriteLine($"txtTimer null? {txtTimer == null}");
            Console.WriteLine($"txtMessage null? {txtMessage == null}");
            ...
            btnGamemode = this.FindControl<Button>("btnGamemode");
        ...
        public GameWindow() : this(new DBController())
        {
            // This is ONLY for the previewer to work — avoid using this constructor at runtime
        }
        ...
        private void InitializeGame()
        {
            ...
            // apply theme and start first game
            theme = new Theme();
            theme.setTheme(this);
            StartNewGame();
        }
        ...
        private void StartNewGame()
        {
            ...
            if (gameMode == 1)
            {
                timedWon = false;
                gamePaused = true;
                txtGameInfo.Text += "   |   Timed";
                txtMessage.Text = "Welcome to Timed Mode!\nYou have one minute to guess the word.\n\nPress START when you're ready!";
                game.maxHints = 0;
            ...
            try
            {
                ...
                else if (letter == '\r')
                {
                    ...
                    if (won)
                    {
                        ...
                        SetHighScore();
                        gameOver = true;
                        txtMessage.Text = $"Congratulations! You guessed it!\nTime: {txtTimer.Text}\nPress any key to start a new game.";
                    }
                    else if (game.board.activeRow < game.board.NUM_ROWS)
                    {
                        txtMessage.Text = "Wrong guess. Try again!";
                    }
                    else
                    {
                        timer.Stop();
        ...
        public void UpdateUsedLetterBoard()
        {
            ...
            foreach (var row in rows)
            {
                ...
                foreach (Button b in row.Children.OfType<Button>())
                {
                    ...
                    foreach (var used in game.usedLetters)
                    {
                        if (used.character == ch)
                        {
        ...
        {
            var menu = btnSettings.ContextMenu;
        ...
        {
            var menu = btnGamemode.ContextMenu;
            menu.PlacementTarget = btnGamemode;
            menu.Open();
        }
